...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    let arrstart = dynamicArray[0];
    for(let i = 1; i < dynamicArray.length; i++) { 
        arrstart = multiply(arrstart, dynamicArray[i])[0];
    } 
    const anyArray = [`The numbers ${dynamicArray} have a product of ${arrstart}.`];
    return [arrstart, anyArray];
}
